...
KEYPAD:
	MOV P2,#11110000B
K1: MOV P2,#11110000B MOV A,P2
	ANL A,#11110000B CJNE A,#11110000B,K1
K2: ACALL DELAY MOV A,P2
	ANL A,#11110000B
...
FIND1:
	RRC A
	JNC MATCH INC DPTR SJMP FIND1
	MA TCH: CLR A
	MOVC A,@A+DPTR
	RET
SEPR:
	JB ACC.6,ALPHA
	JB ACC.4,NUMBER ; Symbol
	CLR R SETB Y SETB G
	MOV R7,#20 LP1: ACALL DELAY
	DJNZ R7,LP1
	SETB R RET
ALPHA:
	CLR G
	SETB Y SETB R
	MOV R7,#20 LP2: ACALL DELAY
	DJNZ R7,LP2
	SETB G RET
NUMBER: CLR Y
	SETB G SETB R
	MOV R7,#20 LP3: ACALL DELAY
	DJNZ R7,LP3
	SETB Y RET
	; 30 msec delay DELAY:
	MOV TMOD,#00000001B MOV TL0,#0CAH
	MOV TH0,#27H
	SETB TR0
BACK: 
	JNB TF0,BACK CLR TR0
	CLR TF0
	RET
